...
async def _all_role_names(db: AsyncSession) -> list[str]:
...
async def get_permissions(
    ...
    roles = await _all_role_names(db)
...
async def update_permissions(
    ...
    valid_roles = set(await _all_role_names(db))
    for item in data.items:
        if item.role == "admin":
            continue  # admin неизменяем
        if item.role not in valid_roles or item.resource not in RESOURCES or item.action not in ACTIONS:
